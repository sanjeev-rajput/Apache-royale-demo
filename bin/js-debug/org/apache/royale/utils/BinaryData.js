 ...
 */
org.apache.royale.utils.BinaryData.prototype.clear = function() {
  this.ba = new ArrayBuffer(0);
  this._position = 0;
  this._len = 0;
 ...
 *  @asparam {uint} writeOffset The optional offset value of the starting bytes to write inside destination
 ...
 * @param {number=} length
 */
org.apache.royale.utils.BinaryData.prototype.readBinaryData = function(destination, writeOffset, length) {
  writeOffset = typeof writeOffset !== 'undefined' ? writeOffset : 0;
  length = typeof length !== 'undefined' ? length : 0;
  if (!destination)
    throw new TypeError('Error #2007: Parameter destination must be non-null.');
  if (destination == this)
    throw new Error('Parameter destination must be another instance.');
  
  if (length == 0)
    length = (this._len - this._position) >>> 0;
  destination.mergeInToArrayBuffer(writeOffset, new Uint8Array(this.ba, this._position, length));
  this._position += (length) >> 0;
};
  ...
  return {
    ...
    methods: function () {
      return {
        ...
        'clear': { type: 'void', declaredBy: 'org.apache.royale.utils.BinaryData'},
